...
locals {
  ...
  _defaults_net = {
    billing_account_id   = local.billing_account.id
    environment_dns_zone = var.environment_dns_zone
    shared_vpc_self_link = try(local.vpc_self_links["dev-spoke-0"], null)
    vpc_host_project     = try(local.host_project_ids["dev-spoke-0"], null)
  }
